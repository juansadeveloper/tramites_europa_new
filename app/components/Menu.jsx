            ...
            <div className="relative w-full max-w-[500px]  mx-4 my-6 md:mx-auto ">
              <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t">
                  ...
                  <div onClick={() => setShowMenu(false)} ><IoClose size={24} className='cursor-pointer text-xl bg-neutral-100 hover:bg-neutral-300 p-1 rounded-full'/></div>
                </div>
                <div className="relative flex-auto p-6 bg-white md:pl-12 w-full">
                  <form className='flex flex-col gap-3 p-5 max-w-full md:max-w-[400px] w-full rounded-md'>
                    <div className='flex flex-col gap-3 md:flex-row md:items-center md:gap-5'>
                      <input className='p-3 rounded text-sm border w-full md:max-w-[180px]' type='text' placeholder='Nombre y apellido' required />
                      <input className='p-3 rounded text-sm border w-full md:max-w-[180px]' type='email' placeholder='E-mail' required />
                    </div>
                    <textarea className='p-3 rounded text-sm border' placeholder='Mensaje' required rows={5}></textarea>
                  </form>
                </div>
                <div className="flex items-center justify-center p-6 border-t border-solid border-gray-300 rounded-b">
                  <button onClick={() => setShowMenu(false)} type="submit" className='w-full max-w-[250px] bg-black text-darkwhite py-3 px-5 border rounded hover:bg-blue hover:text-darkwhite transition duration-300'>
                    Enviar Feedback
                  </button>
                </div>
              </div>
